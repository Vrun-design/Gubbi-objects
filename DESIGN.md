...
- **Home sections:** hero → ticker → the six-object grid → one cobalt spotlight → the fact strip → the first-batch vote → Art Club invitation → three doubts. The fact strip (`club-deal`) is a Kanglish headline beside three hairline-ruled facts on paper-deep: no cards, no numbering, no tilt. The vote grid is paper tiles on coral with an ink pressed state; its note is full-contrast ink, never faded. Do not reintroduce the quiz card, craft steps, scale diagram, places or gifting lists; the home page stays under eight sections.
